...
const AuctionWonCard = ({ item, rating }) => {
  ...
  return (
    <>
              ...
              <div className="relative bg-neutral-primary-soft p-4 md:p-6">
                ...
                <div className="p-4 md:p-5 text-center">
                  ...
                  <h3 className="mb-6 text-body leading-relaxed">
                    {rateType === "uprate" ? (
                      <>
                        Do you want to give a positive rating to the seller{" "}
                      ...
                      <>
                        Do you want to give a negative rating to the seller{" "}
                        ...
                        <span className="mt-2 block text-sm font-medium text-red-600">
                          This action may affect the seller’s ability to
      ...
      >
        {/* Image Section */}
        <div className="w-64 h-full bg-gray-200 flex-shrink-0 relative overflow-hidden border-r border-transparent cursor-pointer">
        ...
        <div className="flex-1 p-5 flex flex-col justify-between">
          ...
          <div className="flex justify-between items-start">
            <div className="pr-4">
              <Link to={`/auctions/${item._id}`}>
                <h3 className="text-lg font-medium text-gray-800 leading-tight line-clamp-2 cursor-pointer">
                  {item.product.name}
                </h3>
              </Link>
              <p className="text-xs text-gray-500 mt-1">
                End On: {formatTime(item.endTime)} - Seller:{" "}
                {item.sellerId.firstName + " " + item.sellerId.lastName}
              </p>
            </div>

            {/* Icons */}
            <div className="flex gap-3 items-center flex-shrink-0 select-none">
              <ThumbsUp
                className={`w-6 h-6 cursor-pointer transition-colors ${
                  (rating && rating.rateType === "uprate") ||
                  (rateType && rateType === "uprate")
                    ? "fill-green-700 text-green-700"
                    : `text-black${isRated ? "" : " hover:text-green-700"}`
                }`}
                strokeWidth={1.5}
              ...
              <ThumbsDown
                className={`w-6 h-6 cursor-pointer transition-colors ${
                  ...
                  (rateType && rateType === "downrate")
                    ...
                    : `text-black${isRated ? "" : " hover:text-red-700"}`
                }`}
                strokeWidth={1.5}
          ...
          <div className="mt-2 w-full">
            <div className="flex flex-col items-end gap-2 w-full">
              ...
              <button
                className={
                  isRated
                    ? "bg-grayinput text-gray-600 px-6 py-1.5 text-sm rounded-md font-medium transition-colors"
                    : "bg-primary hover:bg-accent hover:text-dark text-white px-6 py-1.5 text-sm rounded-md font-medium transition-colors"
                }
                onClick={() => handleShowModal(!isRated)}
                disabled={isRated}
              >
                {isRated ? "Rated" : "Submit"}
              </button>
            </div>
          </div>
        </div>
