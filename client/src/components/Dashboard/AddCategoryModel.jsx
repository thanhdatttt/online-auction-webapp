...
export default function AddCategoryModel({ isOpen, onClose }) {
  // 2. Access Store
  const { getCategories, createCategories } = useCategoryStore();
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const success = await createCategories(cleanData, rawImage[0]);
      
      console.log(success);
      onClose();
    } catch (error) {
      console.error(error);
      // Store usually handles error toasting, but we can double check here
